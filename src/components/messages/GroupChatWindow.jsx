...
export default function GroupChatWindow({ groupConversation, currentUser, onBack }) {
  ...
  }, [messages, currentUser.email, groupConversation, queryClient]);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {participants.slice(0, 3).map((p, idx) => (
              <Link key={idx} to={createPageUrl("UserProfile", `email=${p.email}`)}>
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center hover:scale-110 transition-transform cursor-pointer border-2 border-[#1a1a2e]">
            ...
            {participants.length > 3 && (
              <div className="text-xs text-gray-400">+{participants.length - 3}</div>
            )}
          </div>
        </div>
      </div>
              ...
              <div key={message.id}>
                ...
                >
                  <div className={`max-w-[70%]`}>
                    {!isOwn && (
                      <div className="text-xs text-gray-400 mb-1 ml-2">{message.sender_name}</div>
                    )}
                    <div
                      ...
                      {isOwn && readCount > 1 && (
                        <>
                          ...
                          <span className="text-xs text-gray-400">
                            Read by {readCount - 1}/{totalParticipants - 1}
                          </span>
                        </>
                      )}
                    </div>
